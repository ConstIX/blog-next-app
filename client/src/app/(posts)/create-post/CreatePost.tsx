				...
				>
					<div className='add-post__input'>
							...
							<label className='add-post__btn btn btn_i'>
								...
								<input
									onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
										e.target.files && e.target.files[0] && setDataPost({...dataPost, image: e.target.files[0]})
									}}
									type='file'
									className='input'
								/>
						...
						<div className='add-post__image'>
							{dataPost.image && (
								<img
									src={URL.createObjectURL(dataPost.image)}
									alt={dataPost.image.name}
								/>
							)}
						</div>
					</div>
					...
					<div className='add-post__input'>
						<div className='add-post__item'>
							...
							<input
								...
								onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDataPost({...dataPost, title: e.target.value})}
								type='text'
								placeholder='Title...'
								className='input'
							/>
							<span
								onClick={() => setDataPost({...dataPost, title: ''})}
								className='add-post__clear'
							>
								{dataPost.title && (
									<svg
										clipRule='evenodd'
					...
					<div className='add-post__input'>
						<div className='add-post__item add-post__item_t'>
							...
							<textarea
								value={dataPost.text}
								onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setDataPost({...dataPost, text: e.target.value})}
								rows={10}
								placeholder='Text...'
								className='input'
							/>
							<span
								onClick={() => setDataPost({...dataPost, text: ''})}
								className='add-post__clear'
							>
								{dataPost.text && (
									<svg
