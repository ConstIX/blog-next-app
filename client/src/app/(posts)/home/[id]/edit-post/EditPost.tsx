...
export default function EditPost() {
   ...
   const [dataPost, setDataPost] = useState<{ title: string, text: string, oldImage: string | null, newImage: string | Blob }>({ title: '', text: '', oldImage: null, newImage: '' })
   ...
   const handleSubmit = () => {
      try {
         ...
         updatedPost.append("title", dataPost.title.length === 0 ? 'Default title...' : dataPost.title)
         updatedPost.append("text", dataPost.text.length === 0 ? 'Default text...' : dataPost.text)
         updatedPost.append('id', id as string)
         updatedPost.append('image', dataPost.newImage)

         mutate(updatedPost as any)
      } catch (error) {
         console.log(error)
      }
   }
      ...
      <section className="add-post">
         <div className="container">

            {
               ...
               data === undefined ? <div className="home__status">Something went wrong!</div> :
                  ...
                  <div className="add-post__input">
                     <div className="add-post__item">
                        ...
                        <label className='add-post__btn btn btn_i'>
                           <span>Add Image</span>
                           <input onChange={(e: React.ChangeEvent<HTMLInputElement>) => { e.target.files && e.target.files[0] && setDataPost({ ...dataPost, newImage: e.target.files[0], oldImage: null }) }} type="file" className="input" />
                        </label>
                     </div>
                     <div className="add-post__image">
                        {dataPost.oldImage && <img src={`http://localhost:3005/${dataPost.oldImage}`} alt='...' />}
                        {dataPost.newImage && <img src={URL.createObjectURL(dataPost.newImage as any)} alt='...' />}
                     </div>
                  </div>
